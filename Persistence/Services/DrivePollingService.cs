...
public class DrivePollingService : BackgroundService
{
    ...
    private async Task PollAllContributorsAsync(CancellationToken cancellationToken)
    {
        ...
        var activeTokensCount = await tokenRepo.GetAllActiveAsync();
        _logger.LogInformation("Polling contributors with active Drive tokens.");

        while (!cancellationToken.IsCancellationRequested)
        {
            var token = await tokenRepo.ClaimNextTokenAsync();
            if (token == null) break;
            ...
            try
            {
                ...
                using (zipStream)
                {
                    ...
                    var activePurchases = await dbContext.DatasetPurchases
                        .Where(p => p.Status == "Processing"
                                 && p.DeliveryEndpoint != null
                                 && p.DeliveryEndpoint != string.Empty)
