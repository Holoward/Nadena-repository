...
public class AuthService : IAuthService
{
    ...
    public async Task<ServiceResponse<string>> ForgotPasswordAsync(string email)
    {
        ...
        // Purge expired and used records to prevent unbounded table growth
        var cutoff = DateTime.UtcNow.AddDays(-7);
        var staleRecords = _identityContext.PasswordResetRequests
            .Where(r => r.ExpiresAt < cutoff || r.UsedAt != null);
        _identityContext.PasswordResetRequests.RemoveRange(staleRecords);
    ...
    private string GenerateJwtToken(ApplicationUser user, UserSession session)
    {
        ...
        var secretKey = _configuration["NadenaSettings:JwtKey"];
        ...
        var token = new JwtSecurityToken(
            _configuration["NadenaSettings:JwtIssuer"],
            _configuration["NadenaSettings:JwtAudience"],
            claims,
            expires: expires,
    ...
    private int GetExpiryMinutes()
    {
        ...
        var expiryConfig = _configuration["NadenaSettings:JwtExpireMinutes"];
