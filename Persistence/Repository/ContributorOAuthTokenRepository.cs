...
public class ContributorOAuthTokenRepository : IContributorOAuthTokenRepository
{
    ...
    public async Task<ContributorOAuthToken?> ClaimNextTokenAsync()
    {
        var now = DateTime.UtcNow;
        var token = await _context.ContributorOAuthTokens
            .Where(t => t.IsActive
                && t.EncryptedRefreshToken != string.Empty
        ...
        token.LockedUntil = now.AddMinutes(10);
        _context.ContributorOAuthTokens.Update(token);
        await _context.SaveChangesAsync();
        return token;
    }
