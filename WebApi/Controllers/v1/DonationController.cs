...
public class DonationController : ControllerBase
{
    ...
    public async Task<IActionResult> GetAllDonations([FromQuery] string? format)
    {
        ...
        var records = donations.Select(d =>
        {
            ...
            var getVolumeLong = (string prop) => root.TryGetProperty("volume", out var vol) && vol.TryGetProperty(prop, out var p) && p.TryGetInt64(out var v) ? v : 0L;
            var getVolumeDouble = (string prop) => root.TryGetProperty("volume", out var vol) && vol.TryGetProperty(prop, out var p) && p.TryGetDouble(out var v) ? v : 0.0;
            var getString = (string prop) => root.TryGetProperty(prop, out var p) && p.ValueKind == JsonValueKind.String ? p.GetString() : null;
            var getCollectionPeriodInt = (string prop) => root.TryGetProperty("collectionPeriod", out var cp) && cp.TryGetProperty(prop, out var p) && p.TryGetInt32(out var v) ? v : 0;
            ...
            return new
            {
                ...
                earliest,
                latest,
                totalDaysSpan = getCollectionPeriodInt("totalDaysSpan"),
                totalVideos = getVolumeLong("totalVideos"),
                uniqueVideos = getVolumeLong("uniqueVideos"),
                uniqueChannels = getVolumeLong("uniqueChannels"),
                repeatViewRate = getVolumeDouble("repeatViewRate"),
