        ...
        if (stripeEvent.Type == "checkout.session.completed")
        {
            ...
            if (session != null)
            {
                try
                {
                    ...
                    {
                        _logger.LogError("Webhook unrecoverable: missing metadata in Stripe session {SessionId}", session.Id);
                        return Ok();
                    }
                    ...
                    {
                        _logger.LogError("Webhook unrecoverable: invalid metadata format in Stripe session {SessionId}", session.Id);
                        return Ok();
                    }
                    ...
                    if (dataset == null)
                    {
                        _logger.LogError("Webhook unrecoverable: dataset {DatasetId} not found", datasetId);
                        return Ok();
                    }
                    ...
                    if (buyer == null)
                    {
                        _logger.LogError("Webhook unrecoverable: buyer {BuyerId} not found", buyerId);
                        return Ok();
                    }
                    ...
                    var pdfBytes = _licensePdfService.GenerateLicensePdf(buyer.CompanyName ?? "Data Client", buyer.CompanyName ?? "Data Client", dataset.Id, purchase.PurchasedAt);
