...
public static class IdentityServiceExtensions
{
    ...
    {
        var secretKey = configuration["NadenaSettings:JwtKey"];
        if (string.IsNullOrWhiteSpace(secretKey))
        {
            throw new InvalidOperationException("JWT SecretKey is not configured. Please set NadenaSettings:JwtKey in configuration.");
        }
        ...
        }).AddJwtBearer(o =>
        {
            ...
            o.TokenValidationParameters = new TokenValidationParameters
            {
                ...
                ValidIssuer = configuration["NadenaSettings:JwtIssuer"],
                ValidAudience = configuration["NadenaSettings:JwtAudience"],
            ...
            o.Events = new JwtBearerEvents()
            {
                OnTokenValidated = async context =>
                {
                    ...
                    var cacheKey = $"session_cache_{userId}_{sessionId}_{securityStamp}";
                    
                    if (cache.TryGetValue(cacheKey, out bool isValidSession) && isValidSession)
                    {
                        return; // Valid session from cache
                    }
                    ...
                    // If valid, cache the success for 2 minutes
