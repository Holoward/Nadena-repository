...
public class NadenaE2ETests
{
    ...
    public NadenaE2ETests(WebApplicationFactory<Program> factory)
    {
        ...
        // JWT config required by IdentityServiceExtensions — test-only values, never used in production
        Environment.SetEnvironmentVariable("NadenaSettings__JwtKey", "TestOnlyJwtKey_32CharsMinimum_NotForProd!");
        Environment.SetEnvironmentVariable("NadenaSettings__JwtIssuer", "Nadena");
        Environment.SetEnvironmentVariable("NadenaSettings__JwtAudience", "NadenaUsers");
        Environment.SetEnvironmentVariable("NadenaSettings__JwtExpireMinutes", "60");
        // Stripe placeholder so StripeClient doesn't throw
        Environment.SetEnvironmentVariable("NadenaSettings__StripeSecretKey", "sk_test_placeholder_key_32chars_min");
        // Disable rate limiting in tests to prevent false 429 failures
