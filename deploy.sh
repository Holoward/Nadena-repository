...
if [ -f /etc/nadena/environment ]; then
    set -a
    source /etc/nadena/environment
    set +a
else
    echo "ERROR: /etc/nadena/environment not found. Cannot run migrations." >&2
    exit 1
fi

export ASPNETCORE_ENVIRONMENT=Production

dotnet ef database update --project Persistence --startup-project WebApi --connection "$ConnectionStrings__DefaultConnection"
dotnet ef database update --project Persistence --startup-project WebApi --context NadenaIdentityDbContext --connection "$ConnectionStrings__IdentityConnection"
